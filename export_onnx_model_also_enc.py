...
from segment_anything import build_sam, build_sam_vit_b, build_sam_vit_l, sam_model_registry
...
def export_encoder(sam: Sam, output: str, opset: int, encoder_data_file: str):
    # 原本只有batc是dynamic axis, 我嘗試改成只有color不是
    # dynamic_axes = {
    #     "x": {0: "batch"},
    # }
    dynamic_axes = {"x": {0: "batch", 2: "width", 3: "height"}}

    dummy_inputs = {
        "x": torch.randn(
            1, 3, 1024, 1024, dtype=torch.float
        ),  # TODO: maybe only need to adj this? recall the method to let onnx accept differ size
    }
